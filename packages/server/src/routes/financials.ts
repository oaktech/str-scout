...
financialsRouter.get('/:id/acquisition', dbGuard, async (req, res) => {
  const pool = getPool()!;
  const { rows } = await pool.query('SELECT * FROM acquisition_costs WHERE property_id = $1', [req.params.id]);
...
financialsRouter.get('/:id/financing', dbGuard, async (req, res) => {
  const pool = getPool()!;
  const { rows } = await pool.query('SELECT * FROM financing WHERE property_id = $1', [req.params.id]);
  if (rows.length === 0) return res.status(404).json({ error: 'Not found' });
  res.json(rows[0]);
});
...
financialsRouter.get('/:id/income', dbGuard, async (req, res) => {
  const pool = getPool()!;
  const { rows } = await pool.query('SELECT * FROM rental_income WHERE property_id = $1', [req.params.id]);
  if (rows.length === 0) return res.status(404).json({ error: 'Not found' });
  res.json(rows[0]);
});
